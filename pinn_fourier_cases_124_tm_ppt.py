"""
Case 1 / 2 / 4 — classical Fourier conduction PINN (Fourier law q* = -theta*_x),
same default training preset as `pinn_case0_fourier_unsupervised.py` (use `--fast` for lighter runs).

FD reference: ``simulate_model("Fourier", library_name)``.

Examples:
    python pinn_fourier_cases_124_tm_ppt.py --case double_pulse
    python pinn_fourier_cases_124_tm_ppt.py --all
    python pinn_fourier_cases_124_tm_ppt.py --case heat_cool --fast
"""
...
from pinn_cv_tm_ppt_core import evaluate_on_grid, gaussian_torch, train_pinn_fourier
...
def plot_comparison(
    ...
    axes[0].set_title(f"{title} | PINN Fourier law (unsupervised)")
    ...
    fig.tight_layout()
    fig.savefig(outdir / f"{case_tag}_fourier_pinn_vs_fd.png", dpi=200)
    plt.close(fig)


def run_one_case(case_key: str, device: str, train_kw: dict, t_end: float) -> None:
    ...
    print(f"\n=== {spec['title']} | Fourier law PINN ===")

    outdir = Path("results") / f"pinn_fourier_{tag}"
    outdir.mkdir(parents=True, exist_ok=True)

    model, meta = train_pinn_fourier(
        t_end=t_end,
    ...
    try:
        ...
        print(f"Figure: {outdir / f'{tag}_fourier_pinn_vs_fd.png'}")
    except Exception as exc:
        print("Skipping FD plot:", exc)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fourier-law PINN for cases 1, 2, 4 (library tm_ppt).")
    p.add_argument(
        "--case",
...
def main() -> None:
    ...
    print(f"Fourier-law PINN | device={device} fast={args.fast} (default=high-quality) cases={keys}")

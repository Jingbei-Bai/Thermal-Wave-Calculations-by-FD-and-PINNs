function AutoTopOptThin(nelx,nely,volfrac,rmin,maxiter)
% ------------------------------------------------------------
% 2D thermal topology optimization
% Case:
%   - uniform heat generation over the whole domain
%   - plate sink on the right boundary
%   - bi-material design (high-k / low-k)
%   - CV and Thermomass transient temperature responses with left-boundary plate heat flux
%
% Improved version:
%   - density filter
...
% Usage:
%   AutoTopOptThin;
%   AutoTopOptThin(240,140,0.15,1.8,300);
% ------------------------------------------------------------
...
% ---- load vector: uniform heat generation over all elements ----
iF = reshape(edofMat',4*nelx*nely,1);
sF = repmat(FE,nelx*nely,1);
F = sparse(iF,1,sF,ndof,1);

% ---- BC: plate sink on the right boundary ----
midy = round((nely+1)/2);
sink_half_width = max(1, round(0.10*(nely+1)));  % plate segment on right boundary
...
while change > 1e-3 && loop < maxiter
    ...
    T = zeros(ndof,1);
    T(freedofs) = K(freedofs,freedofs) \ F(freedofs);
    T(fixeddofs) = 0;

    % ---- objective ----
    c = full(F' * T);

    % ---- sensitivities wrt xPhys ----
    Te = T(edofMat);
    ce = sum((Te*KE).*Te,2);   % element energy

    dc_phys = -penal*(kHigh-kLow) * xPhys.^(penal-1) .* reshape(ce,nely,nelx);
...
figure(1);
saveas(gcf, fullfile(outdir, 'design_final.png'));
...
figure(2); clf;
Tgrid = reshape(T,nely+1,nelx+1);
imagesc(Tgrid);
axis equal off;
colorbar;
title('Temperature field');
saveas(gcf, fullfile(outdir, 'temperature_field.png'));

% ---- CV transient response on optimized design ----
% 2D CV model on the same x-y domain as the topology optimization.
% Use the optimized material distribution for spatially varying conductivity.

tau_q = 0.10;              % strongly hyperbolic relaxation time for Cattaneo–Vernotte
 t_end = 0.20;
 dt = 5e-4;
...
% nodal conductivity from element-wise optimized design
kPhys = kLow + xPhys.^penal * (kHigh - kLow);
rhoCpHigh = 10.0;
rhoCpLow = 1.0;
rhoCpPhys = rhoCpLow + xPhys.^penal * (rhoCpHigh - rhoCpLow);
k_node = zeros(nelx+1,nely+1);
rhoCp_node = zeros(nelx+1,nely+1);
...
for it = 1:nt
    for js = 1:nsub
        ...
        T_cv = T_cv - dt_sub * (dqx + dqy) ./ rhoCp_node;
        qx = qx_new;
        qy = qy_new;
    end
    T_time(:,:,it) = T_cv;
...
sgtitle('2D Cattaneo–Vernotte temperature snapshots');
saveas(gcf, fullfile(outdir, 'cv_snapshots.png'));

mid_y = round((nely+1)/2);
figure(4); clf;
imagesc(t_grid, x_cv, squeeze(T_time(:,mid_y,:)));
axis xy;
colorbar;
xlabel('t^*');
ylabel('x^*');
title('CV spacetime at mid-y slice');
saveas(gcf, fullfile(outdir, 'cv_spacetime.png'));
...
for it = 1:nt
    for js = 1:nsub
        ...
        T_tm = T_tm - dt_sub * (dqx + dqy) ./ rhoCp_node;
        qx_tm = qx_new;
        qy_tm = qy_new;
    end
    T_time_tm(:,:,it) = T_tm;
...
saveas(gcf, fullfile(outdir, 'thermomass_snapshots.png'));

figure(6); clf;
imagesc(t_grid, x_cv, squeeze(T_time_tm(:,mid_y,:)));
axis xy;
colorbar;
xlabel('t^*');
ylabel('x^*');
title('Thermomass spacetime at mid-y slice');
saveas(gcf, fullfile(outdir, 'thermomass_spacetime.png'));

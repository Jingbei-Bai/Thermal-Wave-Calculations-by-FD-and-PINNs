...
def dqdx_from_flux_center(q: np.ndarray, q_left: float, q_right: float, dx: float) -> np.ndarray:
    qext = q.copy()
    qext[0] = q_left
    qext[-1] = q_right
    out = np.zeros_like(q)
...
def dqdx_from_flux_rusanov(T: np.ndarray, q: np.ndarray, q_left: float, q_right: float, dx: float, wave_speed: float) -> np.ndarray:
    """Finite-volume divergence using a Rusanov/Lax-Friedrichs numerical flux.

    This is used for the temperature equation T_t + q_x = 0 in the Thermomass branch
    to avoid feeding centered-grid oscillations back into T.
    """
    n = len(T)
    out = np.zeros_like(T)
    a = float(max(wave_speed, 1.0e-12))
    # interface fluxes F_{i+1/2}, total n-1 interior interfaces plus 2 boundary interfaces
    # left boundary at x=0 uses prescribed boundary flux q_left
    ...
    for i in range(n - 1):
        ...
        TL_i = T[i]
        TR_i = T[i + 1]
        F[i + 1] = 0.5 * (qL_i + qR_i) - 0.5 * a * (TR_i - TL_i)
    F[n] = q_right
    out[:] = (F[1:] - F[:-1]) / dx
    return out
    ...
    for n, tn in enumerate(t_full):
        for j in range(nsub):
            ...
            if model == "Fourier":
                ...
                q_new[0] = qL
                q_new[-1] = qR
                dqdx = dqdx_from_flux_center(q_new, qL, qR, dx)
                T_new = T - dt_sub * dqdx
            else:
                if model == "CV":
                    ...
                    # strict PPT thermomass equation; keep existing output paths unchanged.
                    # deprecated old practical tm terms (Gamma_T, Pi_q, eta_tm): not used in this closure.
                    ...
                    u = q / Theta_s
                    dqdx_inst = dqdx_from_flux_center(q, qL, qR, dx)
                    if nx > 2:
